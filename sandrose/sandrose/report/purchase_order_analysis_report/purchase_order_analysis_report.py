...
def get_data(filters):
	...
	query = (
		...
		.select(
			...
			po.company,
			po_item.name,
			po_item.expense_account    #EXPENSE ACCOUNT
		)
	...
	if filters.get("expense_account"):
		query = query.where(po_item.expense_account == filters.get("expense_account"))
	...
	for row in data:
		...
		if filters.get("group_by_po"):
			...
			if po_name not in purchase_order_map:
				...
				# Handle Expense Account properly when grouping
				if row.get("expense_account") != po_row.get("expense_account"):
					po_row["expense_account"] = "Multiple"
